			...
			for( int i = 0; i < vehicles->iVehicleCount; i++ )
			{
				...
				if( gta )
				{
					...
					if( !found )
					{
						...
						// это проебывается и если ты не смотришь на машину и ты далеко то оно в рандом месте на экране рисует ид
						// но мне ПООХУУУУУУУУУУУЙйййййййййййййййййййй
...
void CALLBACK Callback_DrawIDs( std::string param )
{
	...
	{
		shoulddraw = false;
		PrintMessage( "Выключено" );
	}
	else
	{
		shoulddraw = true;
		PrintMessage( "Включено" );
	}
}
